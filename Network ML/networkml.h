...
namespace ML {
	...
	// one part of a multipart/form-data upload, a text field, or a file 
	struct FormPart {
		std::string name;
	...
	// helpers that build Auth and auth header lines for common schemes
	namespace Auth {
	...
	public:
		...
		// modern API: returns a full Response (status + body + headers) `headers` is a list of full header lines, e.g.
		...
		// onProgress, if set, receives (bytesNow, bytesTotal) and returns true on 2xx status
		bool download(std::string url, std::string filePath,
			std::function<void(long, long)> onProgress = {});

		// multipart/form-data upload (file uploads, etc.) and returns a full Response
		Response upload(std::string url, std::vector<FormPart> parts,
			std::vector<std::string> headers = {}, long timeoutSeconds = 0);

		// auto-retry for the verb methods above. Retries on 429 and 5xx, using  Retry-After header when present, 
		// otherwise exponential backoff (baseMs, 2*baseMs, 4*baseMs, ...) (Off by default)
	...
	// conversational LLM client, talks to a local Ollama server by default, or to OpenAI / Anthropic when constructed with a Provider and an API key
	...
	public:
		...
		// key is only used by cloud providers (Ollama and other local backends ignore it) 
		// leave host empty to use the provider's default endpoint (http://localhost:11434 for Ollama)
		...
		// lower-level and stateless, sends the prompt (plus existing history) 
		// returns the full HTTP Response without modifying history.
		Response raw(std::string prompt);

		// Streams the reply token-by-token as it is generated 
		// onToken is called repeatedly with each chunk of answer text
		// On a 2xx response the exchange is appended to history (same as ask())
		// Returns the HTTP status code (200 on success; 0 if the request could not be sent)
		// onReasoning is optional, for reasoning models it receives the model's "thinking" chunks as they stream, separate from the answer
		// Leave it unset to ignore reasoning
		long stream(std::string prompt,
			std::function<void(const std::string&)> onToken,
			std::function<void(const std::string&)> onReasoning = {});

		// The reasoning text from the most recent ask() or stream() (empty for non-reasoning models) 
		// The answer itself is returned by
		// ask() or delivered to onToken if streaming
		...
		// and load it back, so a chat can survive across runs 
		// Both return false on I/O or parse failure
